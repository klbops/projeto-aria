...
router.get('/upcoming',  async (req, res) => { try { res.json(await svc.listUpcoming(req.query.limit)); } catch(e){res.status(500).json({error:e.message})} });
router.get('/today',     async (req, res) => { try { res.json(await svc.listToday()); } catch(e){res.status(500).json({error:e.message})} });
